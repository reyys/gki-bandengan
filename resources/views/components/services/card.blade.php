    ...
    <a href={{ route('services.show', compact('service')) }}>
        <div class="flex max-w-xl flex-col items-start bg-white border h-full">
            <div class="min-h-[200px] w-full">
                <img class="object-cover" src={{ Storage::url($service->image) }} />
            </div>
            <div class="p-5 w-full">
                <div class="flex items-center justify-between w-full">
                    <div class="flex items-center gap-x-4 text-xs w-full">
                        <time datetime="2020-03-16"
                            class="text-gray-500">{{ $service->created_at->diffForHumans() }}</time>
                    </div>
                    <div class="flex items-center space-x-5">
                        ...
                        <form action={{ route('services.destroy', compact('service')) }} method="POST">
                            @csrf
                            @method('DELETE')
                ...
                <div class="group relative">
                    ...
                    <p class="mt-3 line-clamp-3 text-sm leading-6 text-gray-600">{{ $service->content }}
                    </p>
                </div>
            </div>
        </div>
